...
const mockScenarios: Scenario[] = [
  ...
  {
    id: '3',
    name: 'Weekly Data Consistency Check',
    description: 'Validate data consistency across platforms',
    status: 'active',
    format: 'sql',
    sourceConnectorId: '3',
    sourceConfig: JSON.stringify({
      query: 'SELECT * FROM dashboard_metrics WHERE date >= DATE_SUB(CURRENT_DATE(), INTERVAL 7 DAY)',
      database: 'analytics'
    }),
    validationConfig: JSON.stringify({
      comparisons: ['crossPlatform', 'historical'],
      tolerance: 0.05
    }),
    lastRun: '2024-03-19T00:00:00Z',
    steps: 5
  }
];
